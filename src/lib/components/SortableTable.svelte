...
<script lang="ts">
	...
	// Initialize with default sort if specified and update when data or columns change
	$: {
		...
		// Explicitly depend on data to ensure updates when filtered data changes
		if (data) {
			updateSortedData();
		}
	}
				...
				<tr>
					{#each columns as column}
						<th
							...
							aria-label="{column.label}{column.sortable !== false
								? ' - ' + getSortDescription(column.key)
								: ''}"
