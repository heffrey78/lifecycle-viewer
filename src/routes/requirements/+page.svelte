<script lang="ts">
  ...
  import type { Requirement, RequirementFilters, RequirementStatus, RequirementType, Priority } from '$lib/types/lifecycle.js';
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            {#each filteredRequirements as req}
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                  <span class="inline-flex px-2 py-1 text-xs font-semibold rounded border {getStatusColor(req.status)}">
                    {req.status}
                  </span>
                </td>
                <td class="px-6 py-4">
                  <span class="inline-flex px-2 py-1 text-xs font-semibold rounded border {getPriorityColor(req.priority)}">
                    {req.priority}
                  </span>
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  {#if req.risk_level}
                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded {getRiskColor(req.risk_level)}">
                      {req.risk_level}
                    </span>
                  {:else}
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        ...
        <div>
          <p class="text-2xl font-bold text-emerald-600">
            {filteredRequirements.filter(r => ['Implemented', 'Validated'].includes(r.status)).length}
          </p>
          <p class="text-sm text-gray-600">Completed</p>
        </div>
        <div>
          <p class="text-2xl font-bold text-orange-600">
            {filteredRequirements.filter(r => ['In Progress', 'Ready', 'Architecture'].includes(r.status)).length}
          </p>
